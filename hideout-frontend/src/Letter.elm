...
import Http
import Json.Decode as JDec exposing (field, int, string)
import Utils.Types
    exposing
        ( PosIntInput(..)
        , Trio(..)
...
letterJsonDec =
    JDec.map2 Letter (field "body" string) (field "maxReadCount" int)
...
validateInput input =
    input
        |> (\input_ ->
                if String.isEmpty input_.body then
                    Err EmptyBody
        ...
        |> Result.andThen
            (\input_ ->
                case strToPosIntInput input_.maxReadCount of
                    ...
                    Good posInt ->
                        Ok { body = input_.body, maxReadCount = posInt }
            )
