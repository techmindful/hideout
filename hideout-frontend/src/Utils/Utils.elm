...
formatTime targetPosix currentPosix =
    let
        ...
        year =
            toYear utc targetPosix

        month =
            toMonth utc targetPosix |> monthToInt

        day =
            toDay utc targetPosix

        hour =
            toHour utc targetPosix

        min =
            toMinute utc targetPosix

        sec =
            toSecond utc targetPosix

        date =
            String.join "/" <| List.map prefixZero [ year, month, day ]

        time =
            String.join ":" <| List.map prefixZero [ hour, min, sec ]
...
intToOrdStr n =
    let
        postfix =
            ...
            else if modBy 10 n == 1 then
                "st"

            else if modBy 10 n == 2 then
                "nd"

            else if modBy 10 n == 3 then
                "rd"
